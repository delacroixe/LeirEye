...
class PacketCaptureService:
        ...
        try:
            ...
            if packet_info:
                ...
                if self.stats_manager.stats['total'] % 10 == 0:
                    logger.info(f"Paquetes capturados: {self.stats_manager.stats['total']}")
        except Exception as e:
    ...
    def _run_sniff(self):
        ...
        try:
            ...
            sniff(
                ...
                store=False,
                promisc=False,
                stop_filter=lambda x: not self.is_running or self.stats_manager.stats['total'] >= self.max_packets,
                timeout=None
            )
            logger.info(f"✓ Captura finalizada. Total paquetes: {self.stats_manager.stats['total']}")
    ...
    def stop_capture(self) -> CaptureStats:
        ...
        logger.info(f"✓ Captura detenida. Total paquetes: {self.stats_manager.stats['total']}")
