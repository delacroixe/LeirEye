...
async def get_active_connections(
    status: Optional[str] = Query(None, description="Filtrar por status: ESTABLISHED, TIME_WAIT, etc."),
    limit: int = Query(100, ge=1, le=500, description="Máximo de conexiones a retornar")
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=403,
            detail="Permisos insuficientes. Ejecutar backend con sudo."
        )
    except Exception as e:
