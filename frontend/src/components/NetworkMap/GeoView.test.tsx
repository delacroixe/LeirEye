...
jest.mock('react-leaflet', () => ({
  ...
  Polyline: ({ positions, color }: any) => (
    <div data-testid="polyline" data-positions={JSON.stringify(positions)} data-color={color}></div>
  ),
  useMap: () => ({
    fitBounds: jest.fn(),
  }),
...
describe('GeoView', () => {
  ...
  test('renderiza la capa de tiles con URL correcto', () => {
    ...
    expect(tileLayer).toHaveAttribute('data-url', 'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png');
  });
  ...
  test('filtra nodos sin geolocalización', () => {
    ...
    render(<GeoView mapData={dataWithMissingGeo} userLocation={userLocation} />);
